...
import io.upit.resteasy.guice.security.authorizers.DenyAllMethodAuthorizer;
import io.upit.resteasy.exceptions.ResourceException;
import io.upit.resteasy.guice.providers.AuthSessionProvider;
...
public class UploadedFileResource extends AbstractResource<UploadedFile, Long> {
    ...
    @Inject
    public UploadedFileResource(final UploadedFileService uploadedFileService, AuthSessionProvider authSessionProvider) {
        super(UploadedFile.class, uploadedFileService);
    ...
    public Response download(@PathParam("shortHash") String shortHash) {
        ...
        try (final InputStream fileInputStream = uploadedFileService.getFileStream(uploadedFile); ){
            ...
            String urlFileName = URLEncoder.encode(uploadedFile.getFileName(), "UTF-8");

            // If the type is unknown or is not an image, force the user to download the file.
            if (null == uploadedFile.getContentType() || !uploadedFile.getContentType().startsWith("image")) {
                response.header("Content-Disposition", "attachment; filename=\"" + urlFileName + "\"");
            }
